...
resource "vsphere_host_virtual_switch" "vSwitch0" {
   ...
   network_adapters = ["vmnic1", "vmnic0"]
   active_nics = ["vmnic0"]
   standby_nics = ["vmnic1"]
}
...
resource "vsphere_host_port_group" "vm_network" {
    name = "VM Network"
    virtual_switch_name = vsphere_host_virtual_switch.vSwitch0.name
    host_system_id = data.vsphere_host.h1.id
    vlan_id = 0
}

resource "vsphere_host_port_group" "mgmt_network" {
...
resource "vsphere_vnic" "vmk0" {
  ...
  portgroup = vsphere_host_port_group.mgmt_network.name
  mtu = 9000
...
resource "vsphere_vnic" "vmk1" {
  host = data.vsphere_host.h1.id
  portgroup = vsphere_host_port_group.vm_network.name
  mtu = 9000
  netstack = "defaultTcpipStack"
  ipv4 {
    dhcp = true
